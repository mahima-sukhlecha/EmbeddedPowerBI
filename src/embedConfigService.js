...
const rolesWithUserIds = require('./roles.json')
...
async function getEmbedParamsForMultipleReports(workspaceId, reportIds, additionalDatasetIds) {
    ...
    for (const reportId of reportIds) {
        const reportInGroupApi = `https://api.powerbi.com/v1.0/myorg/groups/${workspaceId}/reports/${reportIds}`;
        const headers = await getRequestHeader();
 ...
 * @return Role
 */
function fetchRolesOnBasisOfUsernames(username) {
    if (rolesWithUserIds.WH_Roles.user_mail_Ids.includes(username)) {
        roles = rolesWithUserIds.WH_Roles.roles
    } else if (rolesWithUserIds.PB_Roles.user_mail_Ids.includes(username)) {
        roles = rolesWithUserIds.PB_Roles.roles
    } else {
        roles = ["Employee"]
    }
    return roles
